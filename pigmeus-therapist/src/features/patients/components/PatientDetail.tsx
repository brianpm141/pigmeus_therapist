    ...
    <FloatingFormContainer visible={visible} onClose={onClose} iconName='airline-seat-flat-angled' title={t('common.details')}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="flex-row justify-between mb-8 gap-x-2">
          ...
          <MetricBox label={t('data.weight')} value={patient.physicalMetrics?.weight ? `${patient.physicalMetrics.weight}` : '--'} unit="kg" />
          <MetricBox label={t('data.height')} value={patient.physicalMetrics?.height ? `${patient.physicalMetrics.height}` : '--'} unit="m" />
        </View>

        {/* SECCIÓN: INFORMACIÓN PERSONAL */}
        <SectionHeader title={t('data.personalInfo')} icon="person" iconColor={colors.primary} />
        <View className="bg-surface-light dark:bg-surface-dark rounded-3xl p-5 mb-6 shadow-sm">
        ...
        <SectionHeader title={t('data.medicalInfo')} icon="medical-services" iconColor={colors.primary} />
        ...
        <SectionHeader title={t('FormPatient.emergencyContact')} icon="emergency" iconColor={colors.primary} />
        <View className="bg-surface-light dark:bg-surface-dark rounded-3xl p-5 mb-6 shadow-sm">
...
const SectionHeader = ({ title, icon, iconColor }: { title: string, icon: any, iconColor: string }) => (
...
const InfoRow = ({ label, value, last, isPhone, onPress, iconColor }: { label: string, value: string, last?: boolean, isPhone?: boolean, onPress?: () => void, iconColor?: string }) => (
  <TouchableOpacity
    disabled={!onPress}
    onPress={onPress}
  ...
  >
    ...
    <View className="flex-row items-center flex-1 justify-end ml-4">
      ...
      {isPhone && iconColor && <MaterialIcons name="chevron-right" size={16} color={iconColor} className="ml-1" />}
    </View>
  </TouchableOpacity>
);